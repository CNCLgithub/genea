...
from mlr.share.projects.navigation.utils.vlm_utils import OpenAIUtils
...
class OpenAIPlanner(VLMPlanner):
    def __init__(self, exp_details_filepath, out_filepath, img_filepaths_list):
        ...
        FileUtils.create_file(self._out_filepath)
    ...
    def _run_openai_model(self, openai_message, max_tries=5):
        ...
        while True:
            try:
                openai_response = self.openai_client.chat.completions.create(model=CoreConfig.OPENAI_MODEL_NAME,
    ...
    def run_experiment(self):
        ...
        for img_filepaths in self._img_filepaths_list:
            ...
            trial_name = FileUtils.get_basename(img_filepaths[0], is_attached=False)[0]
    ...
    def record_responses(self, img_filepath, openai_response):
        trial_name = FileUtils.get_basename(img_filepath, is_attached=False)[0]
