...
class FileUtils:
    ...
    def read_file(filepath):
        if not FileUtils.is_file(filepath):
            Msg.print_error("ERROR [FileUtils]: The entered filepath does not exist.")
            assert False

        filename, file_extension = FileUtils.get_basename(filepath, False)
        if file_extension == '.csv':
            return FileUtils.read_csv_file(filepath)
