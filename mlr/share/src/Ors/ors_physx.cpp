...
PhysXInterface::~PhysXInterface() {
  //if(s->connection) s->connection->release();
  delete s;
}
...
void PhysXInterface::setArticulatedBodiesKinematic(uint agent){
  ...
  for(mlr::Body *b: world.bodies) {
    ...
    if(b->type==mlr::BT_dynamic)
      ...
      ((PxRigidDynamic*)s->actors(b->index))->setSolverIterationCounts(40, 20);

      //cout << "SOLVER ITERATION COUNTS ###############################################################" << endl;
      //PxU32 pI;
      //PxU32 vI;
      //((PxRigidDynamic*)s->actors(b->index))->getSolverIterationCounts(pI, vI);
      //cout << pI << endl;
      //cout << vI << endl;
  }
}
...
//      actor = mPhysics->createRigidDynamic(OrsTrans2PxTrans(b->X));
  ...
  if(b->type == mlr::BT_dynamic) {
    ...
    ///***ADDED****////
    actor->setLinearDamping(8);
    actor->setAngularDamping(8);
...
void PhysXInterface::pullFromPhysx(double tau) {
  for_list(PxRigidActor, a, s->actors) {
    ...
    if(a->getType() == PxActorType::eRIGID_DYNAMIC) {
      ...
      b->vel.set(vel[0], vel[1], vel[2]);
      b->angvel.set(angvel[0], angvel[1], angvel[2]);
#if 0
